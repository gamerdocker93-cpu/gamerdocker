            ...
            <div v-if="!showPixQRCode">
                <div v-if="setting != null && wallet != null && isLoading === false" class="flex flex-col w-full">
                    <form action="" @submit.prevent="submitQRCode">
                        ...
                        <div class="mt-3">
                            <p class="mb-2 text-gray-500">{{ state.currencyFormat(parseFloat(setting.min_deposit), wallet.currency) }} - <span v-if="parseFloat(setting.max_deposit) > 0">{{ state.currencyFormat(parseFloat(setting.max_deposit), wallet.currency) }}</span><span v-else>Sem limite</span></p>
                            <div class="w-full flex items-center justify-between bg-white dark:bg-gray-900 rounded py-1">
                                <div class="flex w-full">
                                    <input type="text"
                                           ...
                                           :max=\"parseFloat(setting.max_deposit) > 0 ? setting.max_deposit : null\"
    ...
    export default {
        ...
        methods: {
            ...
            submitQRCode: function(event) {
                ...
                const _maxDeposit = parseFloat(_this.setting.max_deposit);
            if (!Number.isNaN(_maxDeposit) && _maxDeposit > 0 && parseFloat(_this.deposit.amount) > _maxDeposit) {
