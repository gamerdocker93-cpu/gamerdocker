<template>
    <div class="block">
            ...
            <div>
                <ul>
                    ...
                    <li
                        ...
                        @click="setPaymentMethod('card', 'stripe')"
                        class="bg-white dark:bg-gray-900 cursor-pointer flex justify-between hover:bg-green-700/20 px-4 py-3 mb-3"
                    >
                        <div class="flex items-center gap-4">
                            <img :src="`/assets/images/stripe.png`" alt="" width="100" onerror="this.style.display='none'" />
                            <p>STRIPE</p>
                        </div>
                        <div class="flex justify-center items-center text-gray-500 gap-4">
                            <i class="fa-solid fa-chevron-right ml-2"></i>
                ...
                <div
                    v-if="!setting.sharkpay_is_enable && !setting.digitopay_is_enable && !isStripeEnabled"
                    class="text-gray-500"
                >
                ...
                <div v-if="setting != null && wallet != null && isLoading === false" class="flex flex-col w-full">
                    <form action="" @submit.prevent="submitQRCode">
                        ...
                        <div class="mt-5 item-selected">
                            ...
                            <div @click.prevent="setAmount(50.0)" class="item" :class="{ active: selectedAmount === 50.0 }">
                            ...
                            <div @click.prevent="setAmount(200.0)" class="item" :class="{ active: selectedAmount === 200.0 }">
        ...
        <!-- STRIPE -->
        <div
            v-if="paymentType === 'card' && paymentGateway === 'stripe' && setting && isStripeEnabled"
            class="relative"
        >
            <div v-if="isLoading" role="status" class="absolute -translate-x-1/2 -translate-y-1/2 top-2/4 left-1/2">
                <svg
            ...
            <div class="flex flex-col w-full" v-if="!isLoading">
                <div class="w-full p-4 bg-white dark:bg-gray-700 rounded mb-3">
                    <div class="flex justify-between">
                        <h2 class="text-lg">Pague com cartão via Stripe</h2>
                        <div class="text-4xl">
                ...
                <div class="w-full p-4">
                    <form action="" @submit.prevent="submitStripe">
                        ...
                        <div class="mt-4" v-if="sessionId && publishableKey">
                            <StripeCheckout :pk="publishableKey" :session-id="sessionId" />
                        </div>

                        <div class="mt-5 w-full flex items-center justify-center" v-else>
                            <button type="submit" class="ui-button-blue w-full">
                                <span class="uppercase font-semibold text-sm">Continuar no Stripe</span>
                            </button>
                        </div>
                        ...
                        <div class="mt-3 text-xs text-gray-500">
                            <p>
                                * Obs: este botão depende do endpoint backend que cria a sessão do Stripe.
                                Se ainda não existir no seu projeto, vamos criar no próximo passo.
                            </p>
                        </div>
                    </form>
                </div>
...
export default {
    ...
    computed: {
        ...
        isStripeEnabled() {
            ...
            return (
                this.setting.stripe_is_enable === true ||
                this.setting.stripe_is_enable === 1 ||
                this.setting.stripe_is_enable === "1"
            );
        },
    },
    ...
    methods: {
        ...
        validateAmount(_toast) {
            if (!this.deposit.amount) {
                _toast.error(this.$t("You need to enter a value"));
                return false;
            }
            const amount = parseFloat(this.deposit.amount);
            const min = parseFloat(this.setting.min_deposit);
            const max = parseFloat(this.setting.max_deposit);

            if (Number.isNaN(amount)) {
                _toast.error("Valor inválido.");
                return false;
            }
            if (amount < min) {
        ...
        submitQRCode() {
            ...
            if (!this.validateAmount(_toast)) return;
        ...
        submitStripe() {
            ...
            const payload = {
                ...
                accept_bonus: this.deposit.accept_bonus,
                currency: this.wallet ? this.wallet.currency : "BRL",
            };

            HttpApi.post("stripe/create-checkout-session", payload)
                .then((response) => {
                    this.publishableKey = response.data.publishableKey || response.data.pk || null;
                    this.sessionId = response.data.sessionId || response.data.id || null;

                    if (!this.publishableKey || !this.sessionId) {
                        _toast.error("Stripe: resposta inválida (faltou publishableKey/sessionId).");
                    }
                    this.isLoading = false;
                })
                .catch(() => {
                    _toast.error("Stripe: endpoint não encontrado ou Stripe não configurado no backend.");
                    this.isLoading = false;
                });
        },
